...
class BertWithGCNAndMWE(nn.Module):
    ...
    def forward(self, input_ids, target_token_idx, attention_mask, adj, adj_mwe, batch, labels=None):
        ...
        gcn_pooled = torch.gather(gcn.clone().to(torch.int64), 1, target_token_idx_for_gather.clone().to(torch.int64)).view(batch, -1)
        ...
        if labels is not None:
            ...
            # TODO got int need long
            loss = loss_fct(logits.view(-1, self.num_labels).cpu(), labels.clone().to(torch.float32).view(-1).cpu())
            return loss
        else:
