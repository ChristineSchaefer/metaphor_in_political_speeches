...
def plot_history(history, name):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(x, acc, label='acc', color="red")
    plt.plot(x, loss, label='loss', color="blue")
    plt.plot(x, f1, label='F1', color="green")
    plt.plot(x, recall, label='recall', color="yellow")
    plt.plot(x, precision, label='precision', color="black")
    plt.legend()
