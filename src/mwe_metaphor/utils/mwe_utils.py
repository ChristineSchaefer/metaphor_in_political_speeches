...
def count_mwes_trofi(mwe_file, metaphor_file):
    # This is for saif MOH dataset which is already tokenized
    # and has only one sentence in each entry of data
    df = pd.read_csv(metaphor_file, sep=None, header=0,
                     engine='python')  # pd.read_csv(metaphor_file, header=0, sep=',')
    ...
    for i in range(len(sentences)):
        ...
        assert len(sent) == len(ps), "Number of words in sentence{} do not match:".format(str(i)) + str(sent) + str(
            len(sent)) + " " + str(len(ps)) + str(ps)
